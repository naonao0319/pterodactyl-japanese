...
@section('content-header')
    <h1>サーバー<small>システム上で利用可能な全サーバー。</small></h1>
    <ol class="breadcrumb">
                        ...
                        <form action="{{ route('admin.servers') }}" method="GET">
                            <div class="input-group input-group-sm">
                                <input type="text" name="filter[*]" class="form-control pull-right"
                                    value="{{ request()->input()['filter']['*'] ?? '' }}" placeholder="サーバー検索">
                                <div class="input-group-btn">
                                    <button type="submit" class="btn btn-default"><i class="fa fa-search"></i></button>
                                    <a href="{{ route('admin.servers.new') }}"><button type="button"
                                            class="btn btn-sm btn-primary"
                                            style="border-radius: 0 3px 3px 0;margin-left:-1px;">新規作成</button></a>
                                </div>
                            </div>
                        </form>
                    </div>
                        ...
                        <tbody>
                            ...
                            @foreach ($servers as $server)
                                <tr data-server="{{ $server->uuidShort }}">
                                    ...
                                    <td><a
                                            href="{{ route('admin.users.view', $server->user->id) }}">{{ $server->user->username }}</a>
                                    </td>
                                    <td><a
                                    ...
                                    <td class="text-center">
                                        @if($server->isSuspended())
                                            <span class="label bg-maroon">凍結中</span>
                ...
                @if($servers->hasPages())
                    <div class="box-footer with-border">
                        <div class="col-md-12 text-center">
                            {!! $servers->appends(['filter' => Request::input('filter')])->render() !!}</div>
                    </div>
                @endif
            </div>
        </div>
